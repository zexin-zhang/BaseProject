...
public class mapSearch {
    ...
    public static void showonecanteen(restaurant restaurant, User user, String Path) throws IOException, InterruptedException {
        ...
        while (true) {
            ...
            System.out.println("餐廳距離您:" + restaurant.distance(user.getLocation())+"公尺");
    ...
    public void random() throws IOException, InterruptedException {
        ...
        System.out.println(user.getName() + "您好，您目前位置在：主選單>當前位置附近餐廳，請選擇所需要功能");
        System.out.println("----------------------------------------------------------------------");
        System.out.print("請問是否要包含黑名單餐廳（Y/N）：");
        String blacklist = scanner.nextLine();
        if (blacklist.equals("Y") || blacklist.equals("y")) {
            black = true;
        } else if (blacklist.equals("n") || blacklist.equals("N")) {
            black = false;
        } else {
            System.out.println("您輸入的內容不正確，請重新輸入");
            Thread.sleep(2000);
            showMenu();
        }
        restaurantPack random_restaurant = new restaurantPack();
        favoritePack favor = new favoritePack();
        for (int i = 0; i < restaurants.restaurantNumber(); i++) {
            if (black) {
                random_restaurant.add(restaurants.getRestaurant(i));
            } else {
        ...
        if (random_restaurant.restaurantNumber()!=0)
        {
//            int random = (int) Math.random()*random_restaurant.restaurantNumber();
            int random = random_num.nextInt(random_restaurant.restaurantNumber())+1;
            showonecanteen(random_restaurant.getRestaurant(random),user,path+">"+subpath+">您選擇的餐廳為");
        }
        else
        {
            System.out.println("您所選擇的範圍無法進行轉盤，請重新再次處理");
        }
